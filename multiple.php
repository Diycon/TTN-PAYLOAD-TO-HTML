...
$html = "  <div class='col-sm-4'>
    <div class='card'>
      <div class='card-body'>
        ...
        <p class='card-text'>
		Temperature: ". $data[$id][0]['Temperature'] . "&deg;<br />
		Humidity: ". round($data[$id][0]['Humidity'], 1, PHP_ROUND_HALF_UP) . "&#37;<br />
		Update: ". $data[$id][0]['Time'] . "</p>
      </div>
    </div>
  </div>";
    if($i == 1 || $i == 4 || $i == 8 ) {
...
<?php
foreach (array_keys($data) as $id) {  
	echo "<div>Device: ". $id . "  </div>
		  <div>Temperature: ". $data[$id][0]['Temperature'] . "&deg;</div>
		  <div>Humidity: ". round($data[$id][0]['Humidity'], 1, PHP_ROUND_HALF_UP) . "&#37;</div>
		  <div>Last update: ". $data[$id][0]['Time'] . "</div>";				
    ...
    data: {     
		labels: [";  
				for($i=$t; $i>=0; $i--){ echo "'" . date('H:i', strtotime($data[$id][$t]['Time'])) . "', ";} 
				echo "],  
        datasets: [{
            label: 'Temperature',
 			yAxisID: 'y-axis-1',
